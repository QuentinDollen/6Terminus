...
class Delivery_Guy(W.Walker):
    ...
    def ajout_marchandise(self, type_transport, nb):
        if type_transport == 'blé':
            self.cargaison_nourriture[0][1] = self.cargaison_nourriture[0][1] + nb
        if type_transport == 'fruits':
            self.cargaison_nourriture[1][1] = self.cargaison_nourriture[1][1] + nb
        if type_transport == 'viandes':
            self.cargaison_nourriture[2][1] = self.cargaison_nourriture[2][1] + nb
        if type_transport == 'olives':
            self.cargaison_produits[2][1] = self.cargaison_produits[2][1] + nb
        if type_transport == 'argile':
            self.cargaison_produits[0][1] = self.cargaison_produits[0][1] + nb
